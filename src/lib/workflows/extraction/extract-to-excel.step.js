...
const getRow = (specimen, animalGroupsById, animalMeasureTypeNamesOrderedByName, analysesWithGroupNamesOrderedByName) => {
  ...
  const row = {
    ...
    contact_humain: hadHumanContact ? 'OUI' : 'NON',
    contact_animal_domestique: hadAnimalContact ? 'OUI' : 'NON',
...
const extractToExcel = async (params = {}, userId = 'd74796c5-2328-44be-be9e-eae0ac8043c4') => {
  ...
  const progressSize = Math.ceil(total / 100)
  ...
  for await (const specimen of dataStream) {
    ...
    if (((i % progressSize) === 0) || (i === 1)) {
      console.info(`Extracted ${i} specimens out of ${total}...`)

      const progress = Math.ceil((i / total) * 90)
      const message = progress <= 1 ? 'Un instant...' : 'Extraction en cours...'
      console.debug(progress, message)

      await writer.write(encoder.encode(JSON.stringify({ 
        progress,
        message: message, // `Extracted ${i} specimens out of ${total}...` 
      }) + '\n'))
    }
  }
