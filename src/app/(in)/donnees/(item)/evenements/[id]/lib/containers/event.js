...
import Toolbar from '@/app/(in)/donnees/(list)/evenements/lib/components/toolbar'
...
import SpecimenInformationSection from '../containers/specimen'
...
<VStack flex={5} ps={[0, null, 2]} justifyContent='flex-start' alignItems='stretch' gap={[0, null, 2]}>
  ...
  <VStack alignItems='stretch' fontSize={['md', null, 'sm']} gap={0}>
    ...
    <SectionHeading label={`Événement no ${eventId}`} isSticky>
      <DeleteEventButton eventId={eventId} visibility={'hidden'} />
    </SectionHeading>
      ...
      <AccordionItem value='dsc'>
        <Box position='relative'>
          <AbsoluteCenter as={HStack} axis='vertical' insetEnd={5}>
            ...
            <IconButton colorPalette='red' variant='surface' rounded='full' size={['xs']} onClick={null}><RxTrash /></IconButton>
          </AbsoluteCenter>
